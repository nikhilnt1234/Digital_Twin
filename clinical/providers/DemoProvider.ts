/**
 * DemoProvider - Rule-based clinical analysis (no network calls)
 *
 * Uses heuristic rules to produce CareSummaryOutput. Safe fallback for
 * MacBook Air and offline scenarios.
 */

import type { CareSummaryOutput, CheckInPayload } from '../types';

const SYMPTOM_PATTERNS: [string, string][] = [
  ['tired', 'fatigue'],
  ['fatigue', 'fatigue'],
  ['exhausted', 'fatigue'],
  ['pain', 'pain reported'],
...
/**
 * Run demo (rule-based) clinical analysis.
 */
export function runDemoAnalysis(payload: CheckInPayload): CareSummaryOutput {
  ...
  const vitals_assessment: string[] = [];
  if (vitals.bp) {
